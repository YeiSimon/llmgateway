...
clickhouse-client --user llmgateway --password llmgateway --multiquery <<'SQL'
CREATE DATABASE IF NOT EXISTS llmgateway;

CREATE TABLE IF NOT EXISTS llmgateway.gateway_logs (
    id               String,
...
CREATE TABLE IF NOT EXISTS llmgateway.cost_rollup_hourly (
...
CREATE MATERIALIZED VIEW IF NOT EXISTS llmgateway.cost_rollup_hourly_mv
TO llmgateway.cost_rollup_hourly AS
SELECT
...
FROM llmgateway.gateway_logs;

CREATE TABLE IF NOT EXISTS llmgateway.provider_health_5m (
...
CREATE MATERIALIZED VIEW IF NOT EXISTS llmgateway.provider_health_5m_mv
TO llmgateway.provider_health_5m AS
SELECT
