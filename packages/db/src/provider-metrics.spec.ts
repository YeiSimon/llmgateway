...
describe("provider-metrics", () => {
	...
	describe("getProviderMetrics", () => {
		...
		it("should return pre-computed routing metrics for a single provider-model", async () => {
			...
			const metric = metrics.get(metricsKey("gpt-4", "openai", undefined));
		...
		it("should handle multiple model-provider combinations", async () => {
			...
			const gptMetric = metrics.get(metricsKey("gpt-4", "openai", undefined));
			...
			const claudeMetric = metrics.get(
				metricsKey("claude-3-5-sonnet", "anthropic", undefined),
			);
		...
		it("should skip mappings with null routing metrics", async () => {
			...
			expect(metrics.size).toBe(1);
			expect(
				metrics.has(metricsKey("claude-3-5-sonnet", "anthropic", undefined)),
			).toBe(true);
			expect(metrics.has(metricsKey("gpt-4", "openai", undefined))).toBe(false);
		});
	...
	describe("getProviderMetricsForCombinations", () => {
		...
		it("should return metrics for specific combinations only", async () => {
			...
			expect(metrics.size).toBe(1);
			expect(metrics.has(metricsKey("gpt-4", "openai", undefined))).toBe(true);
			expect(
				metrics.has(metricsKey("claude-3-5-sonnet", "anthropic", undefined)),
			).toBe(false);
		});
		...
		it("should return metrics for multiple combinations", async () => {
			...
			expect(metrics.size).toBe(2);
			expect(
				metrics.get(metricsKey("gpt-4", "openai", undefined))?.totalRequests,
			).toBe(100);
			expect(
				metrics.get(metricsKey("claude-3-5-sonnet", "anthropic", undefined))
					?.totalRequests,
			).toBe(200);
		});
