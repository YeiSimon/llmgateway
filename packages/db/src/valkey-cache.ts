...
export class ValkeyCache extends Cache {
	...
	public constructor(valkeyClient: Redis) {
		super();
	...
	public async get(
		...
		try {
			...
			const cached = await this.valkeyClient.get(cacheKey);
			...
			if (isAutoInvalidate && tables.length > 0) {
				...
				if (parsed.timestamp < lastModified) {
					await this.valkeyClient.del(cacheKey);
					return undefined;
				}
			}
	...
	private async getLastModified(tables: string[]): Promise<number> {
		try {
			...
			const timestamps = await this.valkeyClient.mget(...keys);
	...
	private async invalidateByTags(tags: string[]): Promise<void> {
		try {
			...
			for (const tag of tags) {
				const tagKeysSet = this.tagPrefix + tag;
				const tagKeys = await this.valkeyClient.smembers(tagKeysSet);
	...
	private async invalidateByTables(tables: string[]): Promise<void> {
		try {
			...
			for (const table of tables) {
				const tableKeysSet = this.tableKeysPrefix + table;
				const tableKeys = await this.valkeyClient.smembers(tableKeysSet);
	...
	private async deleteKeysInBatches(keys: string[]): Promise<void> {
		try {
			for (let i = 0; i < keys.length; i += this.batchSize) {
				...
				if (batch.length > 0) {
					await this.valkeyClient.unlink(...batch);
				}
			}
		} catch (error) {
			logger.error(
